...
display = pygame.display.set_mode((breadth, length))
...
mixer.music.load("Bossa-Antigua.mp3")
mixer.music.play(-1)
...
class Balloon:
    def _init_(self, speed):
        ...
        self.x = random.randrange(margin, breadth - self.a - margin)
        self.y = length - lowerBound
        self.angle = 90
        self.speed = -speed
    ...
    def move(self):
        ...
        if (self.x + self.a > breadth) or (self.x < 0):
            if self.y > length/5:
                self.x -= self.speed*cos(radians(self.angle))
            else:
                self.reset()
        if self.y + self.b < 0 or self.y > length + 30:
            self.reset()

    def show(self):
        pygame.draw.line(display, self.color, (self.x + self.a/2, self.y + self.b), (self.x + self.a/2, self.y + self.b + self.length))
    ...
    def burst(self):
        ...
        if onBalloon(self.x, self.y, self.a, self.b, pos):
            burst_sound=mixer.Sound("balloonburst.mp3")
            burst_sound.play()
            score += 1
            self.reset()
...
def lowerPlatform():
    pygame.draw.rect(display, darkGray, (0, length - lowerBound, breadth, lowerBound))

def showScore():
    scoreText = font.render("Balloons Bursted : " + str(score), True, white)
    display.blit(scoreText, (150, length - lowerBound + 35))
